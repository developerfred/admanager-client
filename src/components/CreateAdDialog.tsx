...
const newAdDataSchema = z.object({
	...
	referrer: z.union([
		z.literal(""),
		z.string().refine((value) => isAddress(value), "Invalid Ethereum address"),
	]),
});
...
const CreateAdDialog: React.FC<CreateAdDialogProps> = ({
	...
	const [errors, setErrors] = useState<
		Partial<Record<keyof NewAdData, string>>
	>({});
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						<div className="space-y-4">
							...
							{renderFormField(
								...
								"Referrer Address",
								"0x...",
								"Enter the Ethereum address of your referrer (if any)",
							)}
						</div>
						<div className="space-y-4">
							<h3 className="text-xl font-semibold text-[#9AEDEF]">
								Ad Preview
							</h3>
							<div className="border border-[#333] rounded-lg p-4 bg-[#1A1A1A]">
								...
								<p className="text-[#D365E3] truncate">
									{newAdData.link || "Your ad link will appear here"}
								</p>
							</div>
						</div>
					</div>

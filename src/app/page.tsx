...
const STYLES = {
	TABS_LIST:
		"grid w-full grid-cols-2 sm:grid-cols-4 mb-6 sm:mb-8 bg-black/50 backdrop-blur-md rounded-full p-1",
...
const EnhancedAdManager: React.FC = () => {
	...
	const calculateNextAdPrice = async (): Promise<bigint> => {
		try {
			...
			const price =
				basePrice *
				Math.pow(1 + CONSTANTS.PRICE_INCREASE_PERCENTAGE, totalAdsCount);
			...
			logDevInfo("Price calculation error:", {
				error,
